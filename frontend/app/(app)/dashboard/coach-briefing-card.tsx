...
const PANEL_LABEL_CLASS = "pl-2 text-xs font-semibold uppercase tracking-[0.18em] text-zinc-500";
const BODY_TEXT_CLASS = "text-[15px] leading-8 text-zinc-700";
...
function BriefingPanel({
  ...
  return (
    <div className="relative rounded-[1.75rem] border border-zinc-100 bg-white/80 px-5 py-5 shadow-[0_1px_0_rgba(255,255,255,0.9)_inset]">
    ...
    return (
      <Card className="border-zinc-200 shadow-sm">
        <CardHeader className="pb-3">
          <div>
            <p className="text-xs uppercase tracking-[0.18em] text-zinc-400">Morning Briefing</p>
        ...
        <CardContent>
          <div className="rounded-2xl border border-dashed border-zinc-200 bg-zinc-50 px-5 py-6">
            ...
            <p className={`mt-2 ${BODY_TEXT_CLASS} text-zinc-500`}>
  ...
  return (
    <Card className="border-zinc-200 shadow-sm">
      ...
      <CardContent className="grid gap-4">
        <BriefingPanel label="Today's Plan" accentClassName="bg-emerald-400">
          <div className="mt-3 grid gap-3 xl:grid-cols-2">
            {briefing.recommendations.map((item, index) => (
              <div
                key={item}
                className="flex gap-3 rounded-2xl border border-zinc-100 bg-zinc-50/80 px-4 py-4"
              >
                <span
                  className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-emerald-100 text-[11px] font-semibold text-emerald-700"
                >
                  {index + 1}
                </span>
